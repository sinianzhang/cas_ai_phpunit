...
use Hausformat\Lib\Service\LanguageService;
use TYPO3\CMS\Extbase\Reflection\ObjectAccess;
use TYPO3\CMS\Fluid\ViewHelpers\Form\AbstractFormFieldViewHelper;
use TYPO3Fluid\Fluid\Core\ViewHelper\Exception;

/**
 * Extended Select View Helper
 *
 * TODO: Document this or figure out if it is still needed
 * / hf-extbase
...
class SelectViewHelper extends AbstractFormFieldViewHelper
{
    ...
    public function initializeArguments(): void
    {
        ...
        $this->registerArgument('size', 'string', 'Size of select field, a numeric value to show the amount of items to be visible at the same time - equivalent to HTML <select> site attribute');
        ...
        $this->registerArgument('defaultOption', 'string', 'the object of the default option (any object can be passed)');
        $this->registerArgument('addEmptyOption', 'boolean', 'An empty Option is Added');
        $this->registerArgument('emptyOptionLabel', 'string', 'The Label of the Emptyoption, if not set it would be --');
    }
    ...
    public function render():string
    {
        ...
        foreach ($options as $key => $option) {
            $options[$key] = $this->languageService->sL($option);
        }

        if (empty($options)) {
            $options = ['' => ''];
        } elseif ($this->hasArgument('addEmptyOption') && $this->arguments['addEmptyOption']) {
            if ($this->hasArgument('emptyOptionLabel')) {
                $options = ['' => $this->arguments['emptyOptionLabel']] + $options;
            } else {
                $options = ['' => '--'] + $options;
            }
        }
    ...
    {
        $value = '';
        if ($this->hasArgument('defaultOption')) {
            $value = $this->arguments['defaultOption'];
        }
        if (is_object($value)) {
            $value = $this->getValueFromOption($value);
        }
        return $value;
    }

    /**
     * get the value from the given option - can be an object or a string
     *
     * @param mixed $option
     *
     * @return string
    ...
    protected function getValueFromOption($option)
    {
        ...
        $currentValue = '';
        // cache the current value to restore it after this function was run
        if ($this->hasArgument('value')) {
            $currentValue = $this->arguments['value'];
        } else {
            $noCurrentValue = true;
        }
        $this->arguments['value'] = $option;

        // call the normal getValue method
        $value = $this->getValue();
        ...
        if ($noCurrentValue) {
            ...
            $this->arguments['value'] = $currentValue;
        }
    ...
    /**
     * bugfix to handle boolean values correctly
     *
     * @param mixed $value
     *
